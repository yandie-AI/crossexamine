...
Menxia hands the agent's **raw session transcript** to models from other vendors and lets them
**veto the commit**.
...
> "menxia_guard.py source grep result: only a print statement, no Popen call"
...
```bash
export MENXIA_SEATS=minimax,deepseek,moonshot          # three vendors = real independence

export MENXIA_MINIMAX_BASE_URL=https://api.minimaxi.com/anthropic
export MENXIA_MINIMAX_API_KEY=...
export MENXIA_MINIMAX_MODEL=MiniMax-M2.7-highspeed
export MENXIA_MINIMAX_TEMPERATURE=1.0                  # high: you want it hunting, not agreeing

export MENXIA_DEEPSEEK_BASE_URL=https://api.deepseek.com/anthropic
export MENXIA_DEEPSEEK_API_KEY=...
export MENXIA_DEEPSEEK_MODEL=deepseek-v4-pro

export MENXIA_MOONSHOT_BASE_URL=https://api.kimi.com/coding
export MENXIA_MOONSHOT_API_KEY=...
export MENXIA_MOONSHOT_MODEL=kimi-k3
export MENXIA_MOONSHOT_TEMPERATURE=off                 # some reasoning models reject temperature
```

Then:

```bash
menxia review                 # review what is staged
menxia install-hook           # or wire it into git so it is not optional
menxia status                 # reviews, vetoes, overrides, bypasses
```
...
```
$ menxia review
menxia: routine change 27fb9cae4b1e -- 1 seat(s)
  ...
  [low] (kept) The comment on menxia_guard.py:87 quotes the user, but the wording is the
...
stated reason, and so does every `MENXIA_EMERGENCY` bypass. Bypassing is allowed. Bypassing
*quietly* is not.
